#对测序reads在指定基因组区间内进行碱基堆叠统计（pileup），并将统计结果转换为适合可视化的像素级数据。
#主要功能包括：
#base_pileup：统计每个位点的A/C/G/T/N碱基数和测序深度（depth），生成pileup矩阵。
#pileup_to_pixels：将pileup结果按图片宽度聚合，得到每个像素的平均碱基分布，便于绘制可视化图。
#pileup_to_pixels_with_ref：结合参考序列，统计每个像素内参考匹配和变异碱基的数量，突出显示变异信息。
#简言之：碱基堆叠与可视化数据准备模块，为覆盖度、变异等可视化提供基础数据。
