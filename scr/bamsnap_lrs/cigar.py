#将CIGAR字符串、MD标签、CS标签等比对信息解析为结构化的“片段”列表（Segment对象列表），
#每个片段详细描述了比对中的一种操作（如匹配、错配、插入、缺失等）及其长度和消耗的碱基数。
#最终得到一个有序的 Segment 列表，每个 Segment 包含如下信息：
#type：片段类型（如 match、mismatch、ins、del 等）
#op：操作符（如 =、X、I、D、M 等）
#length：该片段的长度
#ref_consumed：参考序列消耗的碱基数
#read_consumed：read序列消耗的碱基数
#read_seq：插入时的序列（可选）
